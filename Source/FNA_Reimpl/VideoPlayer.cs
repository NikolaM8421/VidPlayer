// Modified VideoPlayer.cs in order to use the bundled Theorafile
// and to make it use granule positions instead of reading a frame
// every 1/fps seconds.
...
using TheorafileTarget = TheorafileBun;
#endregion

#nullable disable

namespace Celeste.Mod.VidPlayer.FNA_Reimpl
{
	public sealed class VideoPlayer2 : IDisposable 
	{
		private static bool Patch => !VidPlayerModule.Settings.DisableFNAPatch;
		...
		private void GL_initialize()
		{
			...
			vertBuffer = new VertexBufferBinding(
				new VertexBuffer(
					currentDevice,
					((IVertexType) new VertexPositionTexture()).VertexDeclaration,
					4,
		...
		public Texture2D GetTexture()
		{
			...
			if (	State == MediaState.Stopped ||
				theora == IntPtr.Zero ||
				TheorafileTarget.tf_hasvideo(theora) == 0	)
			{
			...
			// Check for the end...
			bool ended = TheorafileTarget.tf_eos(theora) == 1;
			if (audioStream != null)
			{
			...
			if (ended)
			{
				...
				TheorafileTarget.tf_reset(theora);
		...
		public void Play(Video video, string handle)
		{
			checkDisposed();
			...
			if (theora != IntPtr.Zero) {
				TheorafileTarget.tf_close(ref theora);
				theora = IntPtr.Zero;
			}
			...
			TheorafileTarget.tf_fopen(handle, out theora);
			if (theora == IntPtr.Zero)
			{
				throw new System.IO.FileNotFoundException(handle);
			}
			...
			if (TheorafileTarget.tf_hasvideo(theora) == 1)
			{
		...
		{
			int samples = TheorafileTarget.tf_readaudio(
				theora,
				audioDataPtr,
				AUDIO_BUFFER_SIZE
			);
			...
			else if (TheorafileTarget.tf_eos(theora) == 1)
			{
		...
		private void InitializeTheoraStream()
		{
			...
			while (TheorafileTarget.tf_readvideo(theora, yuvData, 1) == 0);

			// Grab the first bit of audio. We're trying to start the decoding ASAP.
			if (TheorafileTarget.tf_hasaudio(theora) == 1)
			{
				int channels, samplerate;
				TheorafileTarget.tf_audioinfo(theora, out channels, out samplerate);
